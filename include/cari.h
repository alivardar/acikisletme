...
int cari ()
{
  ...
  char *choices[] = {
    ...
    _("Receipt "),
    _("Gelen Fatura Ýþlemleri"),
    _("Bill Kesim Ýþlemleri"),
    _("Yeni Cari Hesap Kartonu"),
    _("Cari Hesap Kartonlarý"),
    _("Back"),
    (char *) NULL,
  };
...
/*burada mysql den istenen tablo ve field alaný okunacak ve deðiþkenler düzenlenecek*/
...
for (i=0;i<pgsql_row_miktar;i++)
  ...
  box (my_menu_win, 0, 0);
  //print_in_middle(my_menu_win, 1, 0, 75, _("  F4-> Düzelt   F5-> Ara   F8-> Sil   ESC-> Çýkýþ   Seçiminizi Yapýnýz "), COLOR_PAIR(1));
  print_in_middle (my_menu_win, 1, 0, 75,
		   _("  F4-> Düzelt  F5-> Ara  F8-> Sil  ESC-> Çýkýþ  Seçiminizi Yapýnýz "),
		   COLOR_PAIR (1));
...
/*durum*/
  ...
  attron (COLOR_PAIR (2));
  mvprintw (1, 0,
	    _("                    Cari Hesap Düzenleme Ekraný                                ") );
  baslik_goruntule ();
  attroff (COLOR_PAIR (2));

/*toplam 3 satýr*/
  mvprintw (4, 1, _("No:") );
  mvprintw (4, 15, _("Bilanço no:") ); 
  mvprintw (6, 1, _("Firma Adý:") );
  mvprintw (8, 1, _("Adres:") );
  mvprintw (10, 1, _("Vergi Dairesi:") );
  mvprintw (12, 1, _("Vergi No:") );
...
mvprintw(LINES - 3, 0, _("                     F2->Kaydet                 ESC->Çýkýþ") );
...
/*bakiye ve durum guncelleme yapýlamaz*/
/*bilanco_no*/
g_strstrip(  field_buffer (field[1], 0) ),
/*firma_adi, adres*/
g_strstrip( field_buffer (field[2], 0) ), g_strstrip( field_buffer (field[3], 0) ),
...
/*toplam 3 satýr*/
  ...
  mvprintw(LINES - 5, 0, _("    F5->Makbuz yazdýr") );
  mvprintw(LINES - 3, 0, _("    F2->Kaydet   F3->Yeni  F8->Firma Seçimi   F7->Tahsil/Tediye   ESC->Çýkýþ") ); 
...
if ( pgsql_sorgula(query) ==1 ) { mesaj(sql_sorgulama_hatasi); return 1;}
	...
	case KEY_F (5):
		...
		sprintf(yazi1_str, _("             Sayýn   %s     'dan\n"), g_strstrip(field_buffer(field[5],0)) );
		sprintf(yazi2_str, _("     yalnýz %s "), para_yazdir( g_strstrip(field_buffer(field[7],0) ))    ); 
		if (tahsil==0) sprintf(yazi3_str, _("tediye edilmiþtir.\n") );
		if (tahsil==1) sprintf(yazi3_str, _("tahsil edilmiþtir.\n") );
  ...
  while ((ch != 27))
	...
	case KEY_F (8):	/*sil tusu basýlýrsa f8 */
	  ...
	  if (donen_deger == 1)
		...
		{
		//iptal edilisrse alacagýna yaz 1
		cari_miktar_isle (firma_adi_str, atoll(tutar_str), 1);		
		}
...
for (i=0;i<pgsql_row_miktar;i++)
  ...
  while ((ch != 27))
	...
	case KEY_RIGHT:
	  form_driver (my_form, REQ_RIGHT_CHAR);
	  break;
	case KEY_F (2):	//duzeltme iþleminde kayýt
	  /*Kayýt yapmadan evvel sonraki alana gecilmeli okuma yapmýyor aksi halde */
	  form_driver (my_form, REQ_NEXT_FIELD);
	  /*yýl ay gun alýyor */
	  ...
	  return;
	  break;
	  /* duzeltme iþlemlerinin sonu */
	case KEY_F (7):	/*firma seçimi */
	  secilen = field_goster ("firmalar", "firma_adi", 0);
	  set_field_buffer (field[5], 0, secilen);
	  /*aþaðýdakileri yapýnca refresh yapýyor */
	  form_driver (my_form, REQ_NEXT_FIELD);
