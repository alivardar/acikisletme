...
char *kesilmemis_irsaliye_goster (unsigned long int kayit_no_gelen)
{
char query[2000] = "";
...
char *yazi;
  ...
  sprintf (query,
  "select distinct irsaliye_no, tarih, firma_adi from irsaliyeler where fatura_no='' and tur='1' order by firma_adi limit 15 offset %li",
  kayit_no_gelen);

  if (strlen (g_strstrip(giris)) > 2)
sprintf (query,
"select irsaliye_no, tarih, firma_adi from irsaliyeler where firma_adi like '%%%s%%' and fatura_no='' and tur='1' order by firma_adi limit 15 offset 0",
giris);
...
if ( pgsql_sorgula(query) ==1 ) { mesaj(sql_sorgulama_hatasi); return 1;}
  ...
  print_in_middle (my_menu_win, 1, 0, 75,
		   _("   F5-> Find   ESC-> Exit   Please Select       "),
		   COLOR_PAIR (1));
  ...
  while ((c = getch ()) != 10)
	...
	case KEY_F (5):
	  aranacak_kelime
	    ( _("Ürün adý veya içinde geçen bir deðer giriniz.    Düzgün sýralama için boþ býrakýnýz. ") );
	  sprintf (donen, "%s", kesilmemis_irsaliye_goster (kayit_no_gelen));
...
//ilk acýlýnca en son kayýt uzerine konumlan
  ...
  row_no_buyuk_tur ("faturalar", "1");
  sprintf (query,
  "select row_no, tarih, fatura_no,irsaliye_no, firma_adi, toplam, kdv, toplam_tutar, isk_oran, isk_tutar from faturalar where row_no=%i and tur='1' ",
  row_no - 1);
...
//ara toplam
  ...
  sprintf (query,
  "select malzeme_kodu, aciklama, miktar, birim_tip, kdv_oran, birim_fiyat,tutar from faturakesilen where tarih='%s-%s-%s'  and belge_no='%s' and irsaliye_no='%s' ",
...
if ((durum == 1) || (durum == 2))	//1=yeni ise , 1=duzeltme ise sadece
   {
	...
	if ( bolme.rem == 0) 
		...
		//kesilmis eski irsaliye secilmek istenirse
		if ( field_index(current_field(my_form)) == 5) 
			{
			  set_field_buffer (field[5], 0, kesilmemis_irsaliye_goster (0) );
			  /*aþaðýdakileri yapýnca refresh yapýyor */
			  form_driver (my_form, REQ_NEXT_FIELD);
			  form_driver (my_form, REQ_PREV_FIELD);

			  sprintf (query,
"select malzeme_kodu, aciklama, giden, birim_tip, birim_fiyat, kime from stokgiden where belge_no='%s'  ",
...
//buraya her malýn kdv si hesaplanýp yazýlacak
sprintf(kdv_tutar_kayit_str,"%lli", paradan_yaziya_cevir(field_buffer (field[8], 0)) );
mesaj(kdv_tutar_kayit_str);
*/
//kontrol et yoksa glib null dondu hata veriyor
		  if ((strlen (rtrim(field_buffer (field[i], 0), " ")) > 2)
		    ...
		    {
sprintf (query,
"insert into faturakesilen (tarih, malzeme_kodu, belge_no, irsaliye_no, aciklama, birim_fiyat, miktar, kimden, kdv_oran, kdv_tutar, birim_tip, tutar) values ('%s','%s','%s','%s','%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
			   /*row_no, tarih */
			    tarih,
		...
		//stokgiden tablosuna kayýt iþlemleri bittikten sonra ilgili irsaliye alanýna fatura numarasý iþlenmlidir
	      sprintf (query,
          "update stokgiden set fatura_no='%s' where (belge_no='%s') ",
	      g_strstrip(field_buffer (field[4], 0)), g_strstrip(field_buffer (field[5], 0))  );
		if ( pgsql_sorgula(query) ==1 ) { mesaj(sql_sorgulama_hatasi); return 1;}

		//irslaiyeler kayýt iþlemleri bittikten sonra ilgili irsaliye alanýna fatura numarasý iþlenmlidir
	      sprintf (query,
	...
	case KEY_F (4):	/*sil */
	  ...
	  if (donen_deger == 1)
	    ...
	    sprintf (query, "delete from faturakesilen where belge_no='%s' ",
		g_strstrip(field_buffer (field[4], 0))  );
  ...
  row_no_buyuk_tur ("faturalar", "1");
  sprintf (query,
  "select row_no, tarih, fatura_no,irsaliye_no, firma_adi, toplam, kdv, toplam_tutar from faturalar where row_no=%i and tur='1' ",
  row_no - 1);
...
/*************************************************************/
	...
	case KEY_NPAGE:	/*geri kayýt */
	  ...
	  row_no_kucuk_tur ("faturalar", "1");
	  if (rownno == row_no)
	    break;

//rownno dan kucuk ama en buyuk kayýt bulunacak
	  sprintf (query,
	  "select row_no, tarih, fatura_no,irsaliye_no, firma_adi, toplam, kdv, toplam_tutar,isk_oran, isk_tutar from faturalar where row_no < %i and tur='1' order by row_no desc",
	  rownno);
...
/***********************************************************************/
	...
	case KEY_PPAGE:	/*ileri kayýt */
	  ...
	  //rownno dan kucuk ama en buyuk kayýt bulunacak
	  sprintf (query,
	  "select row_no, tarih, fatura_no,irsaliye_no, firma_adi, toplam, kdv, toplam_tutar, isk_oran, isk_tutar from faturalar where row_no > %i and tur='1' order by row_no asc",
	  rownno);
