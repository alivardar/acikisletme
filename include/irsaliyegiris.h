...
attron (COLOR_PAIR (2));
attron(A_BOLD);
mvprintw (0, 0,  _("                   Waybill Incoming Screen                                     ") );
attroff(A_BOLD);
attroff (COLOR_PAIR (2));

/*toplam 3 satýr*/
  mvprintw (1, 1, _("No:") );
  mvprintw (1, 15, _("date:") );
  mvprintw (1, 24, "/");
  mvprintw (1, 27, "/");
  mvprintw (1, 34, _("Bill.No:") );
  mvprintw (1, 55, _("WB.No:") );
  mvprintw (2, 1, _("From:") );
...
//baslýk
attron (COLOR_PAIR (2));
  mvprintw (3, 0, _(" Material Code          ") );
  mvprintw (3, 17, _(" Name                           ") );
...
//ilk acýlýnca en son kayýt uzerine konumlan
  ...
  row_no_buyuk_tur ("irsaliyeler", "0");
  sprintf (query,
  "select row_no, tarih, fatura_no, irsaliye_no, firma_adi, talan, teden, teslim_adresi, fsevktar, duz_saat, duzenleyen from irsaliyeler where row_no=%i and tur='0' ",
  row_no - 1);
...
if ((durum == 1) || (durum == 2))	//1=yeni ise , 1=duzeltme ise sadece
   {
	...
	case KEY_F (2):  //kaydetme iþlemleri
	  ...
	  if (durum == 1)	//sadece yeni kayýt ise calýs
		...
		{
		  mesaj ( _("Ýrsaliye no. yazmadýnýz.") );
          form_driver (my_form, REQ_NEXT_FIELD);
	      form_driver (my_form, REQ_PREV_FIELD);
		  break;
		}
		...
		//kontrol et aksi takdirde glib null hatasý veriyor
		    ...
		    {			
			sprintf(query, 
			"insert into stokgelen (tarih, malzeme_kodu, belge_no, aciklama, birim_fiyat, gelen, kimden, tur, birim_tip) values ('%s','%s','%s','%s', '%s', '%s', '%s', '0', '%s')",
		...
		"0", g_strstrip( field_buffer (field[7], 0) ), g_strstrip( field_buffer (field[8], 0) ),
	...
	case KEY_F (4):	/*sil */
				...
				if (donen_deger==1)
				{				
				sprintf(query,"delete from stokgelen where belge_no='%s' ", field_buffer(field[5],0) );
			...
			//duzenþeyen
			  ...
			  sprintf (query,
			  "select malzeme_kodu, aciklama, gelen, birim_tip, birim_fiyat, birim_fiyat from stokgelen where tarih='%s-%s-%s'  and belge_no='%s' ",
			  g_strstrip(field_buffer (field[3], 0)), g_strstrip(field_buffer (field[2], 0)),
...
/*********************************************************/
	...
	if (durum!=	0) break;
		...
		row_no_kucuk_tur("irsaliyeler","0");
		if (rownno==row_no) break;

		//rownno dan kucuk ama en buyuk kayýt bulunacak
		sprintf (query,	"select row_no, tarih, fatura_no, irsaliye_no, firma_adi, talan, teden, teslim_adresi, fsevktar, duz_saat, duzenleyen from irsaliyeler where row_no < %i and tur='0' order by row_no desc"
		,rownno);
